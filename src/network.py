    ...
    def forward(self, input, layers=[], encode_only=False, verbose=False):
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model):
                feat = layer(feat)
                if self.layer_index is not None and layer_id == self.layer_index:
                    return feat
...
# for sliding window inference w/ a feature extractor
class InferenceExtractor(torch.nn.Module):
    def __init__(
        self,
    ...
    @torch.no_grad()
    def forward(self, x):
        x = torch.cat([x, self.feat_extractor(x)], dim=1)
        return self.model(x)
